...
if(!empty($_GET['curso']) || !empty($_GET['alumno'])) {
    ...
    profesor_materia as pm ON c.pm_id = pm.pm_id WHERE al.alumno_id = $alumnos AND pm.pm_id = $curso";
    $querye = $pdo->prepare($sqle);
    $querye->execute(array($alumnos,$curso));
      ...
      <div class="row">
        <div class="col-lg-12">
            <div class="bs-component">
                <ul class="list-group">
                    <li class="list-group-item"><span class="tag tag-default tag-pill float-xs-right"><strong>PROMEDIO: <?= formato(promedio($alumnos)); ?></strong></span></li>
                </ul>
            </div>    
        </div>
      </div>
