...
if(!empty($_GET['curso']) || empty($_GET['contenido'])) {
    ...
    $idProfesor = $_SESSION['alumno_id'];

    $sql = "SELECT *,date_format(fecha, '%d/%m/%Y') as fecha FROM examen WHERE contenido_id = $contenido";
          ...
          <div class="tile">
                  <div class="tile-title-w-btn">
                    <h3 class="title"><?= $data['tema']; ?></h3>
                       <p><a class="btn btn-warning icon-btn" href="entregasex.php?curso=<?= $curso; ?>&contenido=<?= $data['contenido_id']; ?>&exa=<?= $data
                       ['examen_id']; ?>"><i class="fa fa-edit"></i>Realizar Entregas</a></p>
                  </div>
